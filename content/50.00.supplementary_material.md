...
We evaluated the type I error rate and calibration of our GLS model using a null model of random traits and genotype data from 1000 Genomes Phase III.
...
We ran the standard procedures for the TWAS approaches (S-PrediXcan and S-MultiXcan), which included: 1) a standard GWAS using linear regression under an additive genetic model, 2) GWAS processing steps such as harmonization and imputation (as defined in [@doi:10.1002/gepi.22346]), and 3) S-PrediXcan and S-MultiXcan analyses.
Details for each of these steps are provided below.

We began by performing a standard quality control (QC) procedure on 5,923,554 variants.
We filtered out variants with missing call rates over 0.01, minor allele frequencies below 1%, minor allele counts below 20, and Hardy-Weinberg equilibrium p-values below 1e-6.
We also removed samples with high sex-discrepancy and high-relatedness (first and second degree).
We included sex and the top 20 principal components as covariates, and then performed an association test on all 1,000 random phenotypes.

Step 2 of the process involves processing GWAS data.
This includes harmonizing and imputing $z$-scores, which are necessary to ensure that SNPs in the prediction models are adequately represented.
The scripts to run these steps are available on GitHub (https://github.com/hakyimlab/summary-gwas-imputation).
...
To identify gene-trait associations with the highest degree of certainty, we applied a transcriptome-wide association study (TWAS) approach.
This method combines gene expression data with GWAS summary statistics to identify gene-trait associations.
The resulting $p$-values were used to identify gene-trait associations with a high degree of certainty.

We tested our GLS model (Equation (@eq:reg:model)) to assess the association between the 987 latent variables (LVs) in MultiPLIER and 1,000 S-MultiXcan results on random phenotypes.
We built a gene correlation matrix specifically for this cohort (see [Methods](#sec:methods:reg)) and compared the GLS results with an equivalent baseline ordinary least squares (OLS) model assuming independence between genes.
Figure @fig:reg:nulls:qqplots shows the distribution of $p$-values of the OLS and GLS models.
The GLS model had a slightly smaller mean type I error rate (0.0558, SD=0.0127) than the OLS model (0.0584, SD=0.0140).
Additionally, the GLS model was able to correct for LVs with adjacent and highly correlated genes, such as LV234 (Figure @fig:reg:nulls:qqplot:lv234), LV847 (Figure @fig:reg:nulls:qqplot:lv847), LV45 (Figure @fig:reg:nulls:qqplot:lv45), and LV800 (Figure @fig:reg:nulls:qqplot:lv800).
In contrast, the OLS model had higher mean type I errors and smaller-than-expected $p$-values in all these cases.

We also identified LVs with a higher-than-expected mean type I error rate for both the GLS and OLS models.
An example is LV914, shown in Figure @fig:reg:nulls:qqplot:lv914.
These LVs had a relatively small number of adjacent genes at the top.
We believe this inflation in the error rate may be due to inaccuracies in the correlation estimates between the individual-level MultiXcan model and its summary-based version (see Methods).
As a result, we flagged those LVs with a type I error rate larger than 0.07 (127 LVs) and excluded them from our main analyses.
...
We simulated two scenarios to test our clustering pipeline.
In the first scenario, the gene-trait association matrix $\mathbf{M}$ (from S-MultiXcan) was used, but it did not contain any structure that could be used to group traits.
The latent variables $\mathbf{Z}$ from the MultiPLIER models were preserved.
In the second scenario, the latent variables $\mathbf{Z}$ were used, but they did not contain any structure that could be used to group traits.
The gene-trait association matrix $\mathbf{M}$ was preserved.

We tested two scenarios to assess the stability of our clustering pipeline.
The first scenario involved shuffling the genes in the gene expression matrix $\mathbf{M}$ for each trait, and then projecting the randomized matrix into the latent space.
For the second scenario, we projected $\mathbf{M}$ into the latent space, and then shuffled the latent variables (LVs) in the projected matrix $\hat{\mathbf{M}}$ for each trait.
We then ran the same clustering pipeline (as described in the "Methods" section) on both scenarios to generate an ensemble of partitions.
Finally, we computed the stability statistics on the ensemble partitions from different algorithms, and the agreement of the final consensus partition with the ensemble.
...
This indicates that the consensus clustering approach is able to identify meaningful trait relationships in the data.
