...
We ran the following standard procedures for the TWAS approaches (S-PrediXcan and S-MultiXcan): 1) a standard GWAS using linear regression under an additive genetic model, 2) GWAS processing steps, such as harmonization and imputation procedures as defined in [@doi:10.1002/gepi.22346], and 3) S-PrediXcan and S-MultiXcan analyses.
Further details are provided below.

We conducted a genome-wide association study (GWAS) using standard quality control procedures.
We removed variants with missing call rates exceeding 0.01, minor allele frequencies below 1%, minor allele counts below 20, and Hardy-Weinberg equilibrium p-values below 1e-6.
We also removed samples with high discrepancies in sex and high relatedness (first and second degree).
We included sex and the top 20 principal components as covariates, and tested for associations between 5,923,554 variants and 1,000 random phenotypes.

Step 2 of this project involved processing the GWAS data.
This included harmonizing the GWAS and imputing $z$-scores, which are necessary to ensure a sufficient overlap with SNPs in the prediction models.
The scripts to perform these steps are available on GitHub at the following URL: https://github.com/hakyimlab/summary-gwas-imputation.
After running these procedures for all 1,000 random phenotypes, a total of 8,325,729 variants were generated, including those with original and imputed $z$-scores.

Finally, we performed transcriptome-wide association studies (TWAS) to identify genetic associations for complex traits through gene expression patterns.

We ran our GLS model (Equation (@eq:reg:model)) to assess the association between each of the 987 LVs in MultiPLIER and the 1,000 S-MultiXcan results on random phenotypes.
To do this, we built a gene correlation matrix specifically for this cohort (see [Methods](#sec:methods:reg)).
We then compared the GLS results with an equivalent baseline Ordinary Least Squares (OLS) model, which assumes independence between genes.
Figure @fig:reg:nulls:qqplots shows the distribution of $p$-values of the OLS and GLS models.
The GLS model had a slightly lower mean type I error rate (0.0558, SD=0.0127) than the baseline OLS model (0.0584, SD=0.0140).
Additionally, $p$-values followed a more uniform distribution than the OLS model.
This indicates that the GLS model was able to correct for LVs with adjacent and highly correlated genes, such as LV234 (Figure @fig:reg:nulls:qqplot:lv234), LV847 (Figure @fig:reg:nulls:qqplot:lv847), LV45 (Figure @fig:reg:nulls:qqplot:lv45), and LV800 (Figure @fig:reg:nulls:qqplot:lv800).
In contrast, the OLS model had higher mean type I errors and smaller-than-expected $p$-values for these cases.

We detected other LVs with higher-than-expected mean type I errors for both GLS and OLS models.
An example is LV914, shown in Figure @fig:reg:nulls:qqplot:lv914.
These LVs had a type I error rate larger than 0.07 (127 LVs) and were excluded from our main analyses.
This inflation in these LVs might be due to inaccuracies in correlation estimates between the individual-level MultiXcan model and its summary-based version (see Methods).
...
We simulated two scenarios in which the input data matrix $\hat{\mathbf{M}}$ (gene-trait associations from PhenomeXcan projected into the latent gene expression representation) had no structure.
The first case was where the gene-trait association matrix $\mathbf{M}$ (from S-MultiXcan) had no meaningful structure to find groups of traits, but the latent variables in $\mathbf{Z}$ from the MultiPLIER models were preserved.
The second case was where the latent variables in matrix $\mathbf{Z}$ had no meaningful structure to find groups of traits, but the gene-trait association matrix $\mathbf{M}$ was preserved.

We used two scenarios to test the stability of our clustering pipeline.
In the first scenario, we shuffled the genes in the gene expression matrix for each trait and then projected the randomized matrix into the latent space.
For the second scenario, we projected the gene expression matrix into the latent space and then shuffled the latent variables for each trait.
We applied the same clustering pipeline to both scenarios (see Methods) to generate an ensemble of partitions, which we combined using consensus functions to derive the final partitions of traits.
We measured the stability of the clustering pipeline by computing 1) statistics on the ensemble partitions from different algorithms, and 2) the agreement of the final consensus partition with the ensemble.
...
This indicates that the consensus clustering approach is able to capture meaningful patterns in the data, and suggests that the projected associations are not random.
