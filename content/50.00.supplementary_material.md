...
We assessed our GLS model type I error rates and calibration using a null model of random traits and genotype data from 1000 Genomes Phase III.
...
To run the TWAS approaches (S-PrediXcan and S-MultiXcan), we used standard GWAS procedures, including linear regression under an additive genetic model, as well as harmonization and imputation procedures as defined in [@doi:10.1002/gepi.22346].
Further details for each of these steps are provided below.

We conducted standard quality control procedures for our genetic studies.
We filtered out variants with a missing call rate above 0.01, a minor allele frequency below 1%, a minor allele count below 20, and a Hardy-Weinberg equilibrium below 1e-6.
We also removed samples with high sex-discrepancy and high-relatedness (first and second degree).
Additionally, we included sex and the top 20 principal components as covariates and performed the association test on 5,923,554 variants across all 1,000 random phenotypes.

Step 2 of the process involved harmonizing and imputing $z$-scores for GWAS.
This is part of the TWAS pipeline and is necessary to ensure that SNPs in prediction models are sufficiently overlapping.
The scripts to run these steps can be found on the GitHub repository 'summary-gwas-imputation'.
This process was run for all 1,000 random phenotypes, resulting in 8,325,729 variants, with both original and imputed $z$-scores.

Finally, we applied the Trans-eQTL Weighted Association Score (TWAS) method to project genetic associations through gene expression patterns, thereby highlighting disease etiology and drug mechanisms.

In the third step of our analysis, we used the Trans-eQTL Weighted Association Score (TWAS) method to project genetic associations from imputed GWAS data through gene expression patterns from 49 tissues from GTEx v8.
This allowed us to identify potential disease etiology and drug mechanisms.
To do this, we processed the GWAS with S-PrediXcan and S

We ran our GLS model (Equation (@eq:reg:model)) to compute an association between each of the 987 latent variables (LVs) in MultiPLIER and the 1,000 S-MultiXcan results on random phenotypes.
We built a gene correlation matrix specifically for this cohort (see [Methods](#sec:methods:reg)) and compared the GLS results with an equivalent baseline ordinarly least squares (OLS) model that assumes independence between genes.
Figure @fig:reg:nulls:qqplots shows the distribution of $p$-values of the OLS and GLS models.
The GLS model had a slightly smaller mean type I error rate (0.0558, SD=0.0127) than the baseline OLS model (0.0584, SD=0.0140), and $p$-values followed the expected uniform distribution more closely.
The GLS model was able to correct for LVs with adjacent and highly correlated genes at the top, such as LV234 (Figure @fig:reg:nulls:qqplot:lv234), LV847 (Figure @fig:reg:nulls:qqplot:lv847), LV45 (Figure @fig:reg:nulls:qqplot:lv45), and LV800 (Figure @fig:reg:nulls:qqplot:lv800).
As expected, the OLS model had higher mean type I errors and smaller-than-expected $p$-values in all these cases.

We detected LVs with higher-than-expected mean type I errors for both the GLS and OLS models.
An example is LV914, shown in Figure @fig:reg:nulls:qqplot:lv914.
These LVs had a type I error rate larger than 0.07 (127 LVs), which we excluded from our main analyses.
This inflation in these LVs might have been caused by inaccuracies in the correlation estimates between the individual-level MultiXcan model and its summary-based version (see Methods).
...
We simulated two scenarios to evaluate our clustering pipeline: one where the gene-trait association matrix $\mathbf{M}$ (from S-MultiXcan) did not contain any meaningful structure to identify groups of traits, while preserving the latent variables in $\mathbf{Z}$ from the MultiPLIER models; and another where the latent variables in matrix $\mathbf{Z}$ did not have any meaningful structure to identify groups of traits, while preserving the gene-trait association matrix $\mathbf{M}$.
In both cases, we assumed that there was no structure in the input data matrix $\hat{\mathbf{M}}$ (gene-trait associations from PhenomeXcan projected into the latent gene expression representation).

We tested two scenarios to assess the stability of the clustering results.
In the first scenario, we shuffled the genes in the matrix $\mathbf{M}$ for each trait and then projected this randomized matrix into the latent space.
In the second scenario, we projected the matrix $\mathbf{M}$ into the latent space and then shuffled the latent variables in the resulting matrix $\hat{\mathbf{M}}$ for each trait.
We ran the same clustering pipeline (as described in the Methods section) for both scenarios, generating an ensemble of partitions that was then combined using consensus functions to derive the final partitions of traits.
We calculated two statistics to assess the stability of the results: 1) the stability of the ensemble partitions from different algorithms, and 2) the agreement of the final consensus partition with the ensemble.
...
This indicates that our method is able to identify meaningful gene expression patterns related to the genetic associations.
