...
We assessed the type I error rate and calibration of our GLS model using a null model of random traits and genotype data from 1000 Genomes Phase III.
We selected 312 individuals with European ancestry and analyzed 1,000 traits drawn from a standard normal distribution $\mathcal{N}(0,1)$.
To do this, we followed the standard procedures for the TWAS approaches (S-PrediXcan and S-MultiXcan), which included: 1) a standard GWAS using linear regression under an additive genetic model, 2) GWAS processing steps such as harmonization and imputation, as defined in [@doi:10.1002/gepi.22346], and 3) S-PrediXcan and S-MultiXcan analyses.

We performed quality control procedures, such as filtering out variants with a missing call rate higher than 0.01, a minor allele frequency (MAF) below 1%, a minor allele count (MAC) below 20, and a Hardy-Weinberg equilibrium (HWE) below 1e-6.
We also removed samples with high sex-discrepancy and high-relatedness (first and second degree).
To account for population structure, we included sex and the top 20 principal components as covariates, and conducted an association test on 5,923,554 variants across all 1,000 random phenotypes.

Step 2 of the process involves processing GWAS data.
This includes harmonizing the GWAS and imputing $z$-scores.
The scripts for these steps are available on GitHub and are part of the TWAS pipeline.
They are necessary to ensure that the SNPs in the prediction models have an acceptable overlap.
When these procedures were run for all 1,000 random phenotypes, 8,325,729 variants were generated, including those with original and imputed $z$-scores.

Finally, we applied TWAS to identify gene-trait associations that are driven by the expression of other genes.

We conducted a third step of the analysis which was called TWAS.
We used S-PrediXcan and MASHR prediction models on 49 tissues from GTEx v8 to process the imputed GWAS.
We then ran S-MultiXcan using the GWAS and S-PrediXcan outputs to generate gene-trait association $p$-values.
Finally, we used TWAS to identify gene-trait associations that are driven

We compared the results of our GLS model (Equation (@eq:reg:model)) with an equivalent, baseline OLS model.
We built a gene correlation matrix for a cohort of 987 LVs and 1,000 S-MultiXcan results on random phenotypes (see [Methods](#sec:methods:reg)).
Figure @fig:reg:nulls:qqplots compares the distribution of $p$-values of the OLS and GLS models.
The GLS model had a smaller mean type I error rate (0.0558, SD=0.0127) than the baseline OLS model (0.0584, SD=0.0140), and $p$-values more closely followed the expected uniform distribution.
...
The OLS model, however, had higher mean type I errors and smaller-than-expected $p$-values in all these cases.

We detected other LVs with higher-than-expected mean type I errors for both the GLS and OLS models, even though they didn't have a large number of adjacent genes at the top.
An example is LV914, shown in Figure @fig:reg:nulls:qqplot:lv914.
This inflation may be due to inaccuracies in correlation estimates between the individual-level MultiXcan model and its summary-based version (see Methods).
We excluded those LVs with a type I error rate larger than 0.07 (127 LVs) from our main analyses.
...
We simulated two scenarios where there was no structure in the input data matrix $\hat{\mathbf{M}}$.
In the first case, the gene-trait association matrix $\mathbf{M}$ (from S-MultiXcan) did not contain any meaningful groupings of traits, while the latent variables in $\mathbf{Z}$ from the MultiPLIER models were preserved.
In the second case, the latent variables in matrix $\mathbf{Z}$ did not contain any meaningful groupings of traits, while the gene-trait association matrix $\mathbf{M}$ was preserved.

We tested two scenarios: shuffling genes in the gene expression matrix $\mathbf{M}$ for each trait, and shuffling the latent variables (LVs) in the projected matrix $\hat{\mathbf{M}}$.
For each scenario, we ran the same clustering pipeline ([Methods](#sec:methods:clustering)) to generate an ensemble of partitions.
We then combined these partitions using consensus functions to derive the final partitions of traits.
To assess the stability and agreement of the final consensus partition, we computed two statistics: (1) stability on the ensemble partitions from different algorithms, and (2) the agreement of the final consensus partition with the ensemble.
...
This indicates that the consensus clustering approach is able to identify meaningful trait pairs that are grouped together across different clustering algorithms and data representations.
