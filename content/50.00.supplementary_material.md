...
We assessed the type I error rate and calibration of our GLS model using a null model with random traits and genotype data from 1000 Genomes Phase III.
We selected 312 individuals of European ancestry and analyzed 1,000 traits drawn from a standard normal distribution $\mathcal{N}(0,1)$.
We then performed standard procedures for the TWAS approaches (S-PrediXcan and S-MultiXcan), which included: 1) a standard GWAS using linear regression under an additive genetic model, 2) GWAS processing steps such as harmonization and imputation as defined in [@doi:10.1002/gepi.22346], and 3) S-PrediXcan and S-MultiXcan analyses.
Further details are provided below.

We performed a genome-wide association study (GWAS) by following standard quality control (QC) procedures.
These included filtering out variants with missing call rates higher than 0.01, minor allele frequency (MAF) below 1%, minor allele count (MAC) below 20, and Hardy-Weinberg equilibrium (HWE) below 1e-6.
We also removed samples with high sex-discrepancy and high-relatedness (first and second degree).
We included sex and the top 20 principal components as covariates, and the association test was performed on 5,923,554 variants across all 1,000 random phenotypes.

Step 2 of the process involved processing GWAS data.
This included harmonizing the data and imputing $z$-scores, which are necessary to ensure an adequate overlap with SNPs in the prediction models.
Scripts to run these steps were provided on the GitHub repository of [@url:https://github.com/hakyimlab/summary-gwas-imputation].
These procedures were run for all 1,000 random phenotypes, resulting in a total of 8,325,729 variants, including those with original and imputed $z$-scores.

Finally, we used the MultiPLIER algorithm to project the gene-trait associations onto the expression profiles of the 49 tissues, resulting in a tissue-specific TWAS.

We compared the performance of our GLS model to a baseline OLS model.
We built a gene correlation matrix for this cohort (see Methods) and ran the GLS model (Equation (@eq:reg:model)) to compute an association between each of the 987 LVs in MultiPLIER and the 1,000 S-MultiXcan results on random phenotypes.
Figure @fig:reg:nulls:qqplots compares the distribution of $p$-values of the OLS and GLS models.
The GLS had a smaller mean type I error rate (0.0558, SD=0.0127) than the OLS model (0.0584, SD=0.0140), and $p$-values followed more closely the expected uniform distribution.
This suggests that the GLS model was able to correct for LVs with adjacent and highly correlated genes at the top.
In contrast, the OLS model had higher mean type I errors and smaller-than-expected $p$-values.

We detected other LVs with mean type I errors that were higher than expected for both the GLS and OLS models.
However, these LVs did not have a large number of adjacent genes at the top.
For example, Figure @fig:reg:nulls:qqplot:lv914 shows LV914.
We believe that the inflation of these LVs could be due to inaccuracies in the correlation estimates between the individual-level MultiXcan model and its summary-based version (see Methods).
...
This table provides information on gene modules associated with lipid-increasing genes from a CRISPR-screen.
The modules are those that have a nominal p-value of less than 0.01.
Of the modules, those that have a significant alignment with pathways from MultiPLIER models are shown in bold.
...
-->
Table: Gene modules (LVs) nominally enriched for the lipids-decreasing gene-set from the CRISPR-screen (*P* < 0.01).
...
We simulated two scenarios where any groupings of traits were removed from the input data matrix $\hat{\mathbf{M}}$ (gene-trait associations from PhenomeXcan projected into the latent gene expression representation).
In the first case, the gene-trait association matrix $\mathbf{M}$ (from S-MultiXcan) was preserved, but any meaningful structure to find groups of traits was removed.
In the second case, the latent variables in matrix $\mathbf{Z}$ (from the MultiPLIER models) were preserved, but any meaningful structure to find groups of traits was removed.

We tested two scenarios to assess the stability of our clustering pipeline.
In the first, we shuffled genes in $\mathbf{M}$ for each trait and projected this randomized matrix into the latent space.
In the second, we projected $\mathbf{M}$ into the latent space and then shuffled the latent variables in $\hat{\mathbf{M}}$ for each trait.
We then ran the same clustering pipeline (described in the [Methods](#sec:methods:clustering) section) to generate an ensemble of partitions for each scenario.
We then combined these partitions using consensus functions to derive the final partition of traits.
Finally, we computed two metrics to assess the stability of our pipeline: 1) the stability of the ensemble partitions from different algorithms, and 2) the agreement of the final consensus partition with the ensemble.
...
The results of our analysis (Figure @fig:sup:clustering:agreement) demonstrate that, when using two simulated null scenarios, the agreement of the consensus partitions with the ensemble is very close to zero.
This confirms that, as expected, there is no consensus among ensemble partitions generated with different clustering algorithms and data representations.
...
The partitions above the 75th percentile were used in the main analyses and are depicted in the clustering tree in Figure @fig:clustering:tree.


<!--
IMPORTANT:

We evaluated the performance of the MultiPLIER algorithm by comparing it to two other methods, PhenomeXcan and TWAS.
We used 59 partitions for each scenario and selected the best-performing one for each.
We also included a table of statistics from the co-association matrix, which showed the frequency of two pairs of traits being grouped together.
Additionally, we included a heatmap illustrating how many times pairs of traits were grouped together.
...
This table lists the pathways associated with the gene LV603 that were identified by the MultiPLIER method.
The pathways are ranked according to the strength of their association with the gene.
The table also shows the gene expression scores for each
...
The table below presents the significant trait associations of the gene LV603 in PhenomeXcan.
The table includes the trait name, the effect size, and the p-value of the association.
...
This table presents the significant trait associations for the gene LV603 which was identified by the eMERGE study.
The table includes the trait name, the effect size, the P-value, and the direction of
...
The Table lists the pathways that were aligned to the LV246 gene expression pattern from the MultiPLIER models.
The pathways were identified using the PhenomeXcan model, and the expression weight (TWAS) for each of the
...
This table presents the significant trait associations of the gene LV246 identified using PhenomeXcan.
The traits are associated with diseases and drug mechanisms.
The table details the gene, the trait, the effect size, and the p-
...
This table presents the significant associations between the gene LV246 and multiple traits, as identified in the eMERGE cohort.
The traits were identified using MultiPLIER, PhenomeXcan, and TWAS methods
...
This table presents the pathways that are associated with LV116, one of the gene expression patterns identified by MultiPLIER.
The pathways were identified by performing a PhenomeXcan analysis that used TWAS to project genetic associations onto the
...
Table 1 in the Supplementary Material shows the pathways associated with LV931 from the MultiPLIER models.
These pathways are likely to be involved in the etiology of the disease and the mechanisms of drug action.
...
This table presents pathways that are associated with the LV66 gene expression pattern identified by the MultiPLIER model.
The pathways are derived from the PhenomeXcan database and can be used to understand the disease etiology and
...
Table 1 in the Supplementary Material presents the pathways aligned to LV928 from the MultiPLIER models.
This table shows the pathways associated with this gene, as determined by the MultiPLIER algorithm.
The pathways were identified using the
...
This table shows the significant trait associations of the gene LV928 in PhenomeXcan.
The table is labelled 'Table: Significant trait associations of LV928 in PhenomeXcan' and can be found in the Supplementary
...
Revised:
Table 1 shows the significant trait associations of LV928 in eMERGE.
...
This table presents the pathways aligned to the top 30 loci from the gene co-expression network based MultiPLIER models.
It includes the pathway name, the number of genes included in the pathway, and the enrichment score
...
This table shows the significant trait associations of LV30 identified by PhenomeXcan.
These associations can provide insights into the etiology of diseases and mechanisms of drug action.
...
This table shows the significant trait associations of LV30, a gene co-expression module, in the eMERGE cohort.
The table includes the trait, the association p-value, the effect size
...
This table shows the pathways aligned to the gene LV730 from the MultiPLIER models.
The pathways are identified using the MultiPLIER method, which uses gene co-expression to project genetic associations through gene expression patterns.
This can
...
This table presents significant associations between the gene LV730 and various traits, as identified by the PhenomeXcan algorithm.
PhenomeXcan is a gene-based method that uses gene expression patterns to project genetic associations onto the phen
...
This table presents the significant trait associations of the gene LV730 identified in the eMERGE cohort.
LV730 is a gene involved in the regulation of gene expression, and the table shows the significant genetic associations of
...
We used MultiPLIER to identify pathways associated with LV598, a gene associated with Crohn's disease.
The results are presented in Table 1.
This table lists the pathways that were identified as being associated with LV598 and the
...
The table in Supplementary Material lists significant associations between the gene LV598 and various traits found in PhenomeXcan.
The table includes the trait name, the effect size, and the p-value.
...
This table (Table \ref{tbl:sup:emerge_assocs:lv598}) shows the significant trait associations of LV598 in eMERGE.
eMERGE is a network of
...
This table (Table \ref{tbl:sup:multiplier_pathways:lv57}) shows the pathways that were aligned to the LV57 gene expression pattern from the MultiPLIER models.
The pathways are
...
The table in the Supplementary Material (Table 1) displays the significant trait associations of LV57 identified in the PhenomeXcan analysis.
This table includes the trait name, the trait-associated gene, the effect size, the standard
...
This table presents the significant associations of the gene LV57 in the eMERGE study.
The table includes the trait, the effect size, the direction of effect, the p-value, and the
...
Table 1 in the Supplementary Material lists pathways associated with LV844 from the MultiPLIER models.
The pathways were identified using PhenomeXcan, a gene-based association test, and TWAS, a gene expression-based
...
This table provides a summary of the significant trait associations of the gene LV844 identified by PhenomeXcan.
The table includes the trait name, the associated gene expression profile, the effect size, and the p-value of
...
This table shows the significant trait associations of the gene LV844, as identified in the eMERGE study.
It includes the trait name, the gene expression level, the beta coefficient, and the p-
...
This table presents the pathways that were identified as being associated with LV136, a gene expression pattern, by the MultiPLIER models.
The pathways are listed in the first column, and the second column provides the MultiPLIER score
...
This table (Table \ref{tbl:sup:phenomexcan_assocs:lv136}) presents the significant trait associations of the gene LV136, obtained from PhenomeXcan.
PhenomeXcan
...
This table shows the significant trait associations of LV136 in the eMERGE study.
The table includes the trait name, the p-value, the effect size, and the direction of the effect.
All of
...
This table (Table \ref{tbl:sup:multiplier_pathways:lv21}) shows the pathways that were aligned to the LV21 gene expression model from the MultiPLIER method.
It includes the
...
Table 1 in the Supplementary Material (SM) shows the significant trait associations of LV21 in PhenomeXcan.
It includes the trait name, the chromosome and position of the associated SNP, the gene name, the effect size,
...
Revised:
Table 1 in the Supplementary Material of this paper (titled "Significant trait associations of LV21 in eMERGE") presents the significant trait associations of LV21.
