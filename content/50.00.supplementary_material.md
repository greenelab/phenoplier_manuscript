...
1) A standard GWAS using linear regression under an additive genetic model: We selected 312 individuals with European ancestry, and then analyzed 1,000 traits drawn from a standard normal distribution $\mathcal{N}(0,1)$.

2) Different GWAS processing steps, including harmonization and imputation procedures as defined in [@doi:10.1002/gepi.22346]: We ran all the standard procedures for the TWAS approaches (S-PrediXcan and S-MultiXcan), including: 1) a standard GWAS using linear regression under an additive genetic model, 2) different GWAS processing steps, including harmonization and imputation procedures as defined in [@doi:10.1002/gepi.22346], 3) S-PrediXcan and S-MultiXcan analyses.
...
We used the $p$-values to identify the most significant associations between the traits and the GWAS signals.
The most significant associations were then used to identify the most significant genes associated with the traits.
...
We used the $p$-values to identify the
...
In both cases, the algorithm finds groups of traits by clustering the gene expression data.

For our clustering pipeline, we simulated different escenarios where there is no structure in the input data matrix $\hat{\mathbf{M}}$ (gene expression data projected into the latent gene expression representation).
For this, we simulated two cases where any groupings of traits are removed: 1) the gene-trait association matrix $\mathbf{M}$ (from PhenomeXcan) does not have any meaningful structure to find groups of traits, while preserving the latent variables in $\mathbf{Z}$ from the MultiPLIER models; and 2) the latent variables in matrix $\mathbf{Z}$ does not have any meaningful structure to find groups of traits, while preserving the gene-trait association matrix $\mathbf{M}$.
In both cases, the algorithm finds groups of traits by clustering the gene expression data.
