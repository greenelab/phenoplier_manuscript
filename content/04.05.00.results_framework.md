...
1) an LV-based regression model to compute an association between an LV and a trait ([Methods](#sec:methods:reg)),
2) a clustering framework to learn groups of traits with shared transcriptomic properties ([Methods](#sec:methods:clustering)),
and 3) an LV-based drug repurposing approach that links diseases to potential treatments ([Methods](#sec:methods:drug)).


We used TWAS results from PhenomeXcan [@doi:10.1126/sciadv.aba2083] and the eMERGE network as discovery and replication cohorts, respectively ([Methods](#sec:methods:twas)).
PhenomeXcan provides gene-trait associations for 4,091 different diseases and traits from the UK Biobank [@doi:10.1038/s41586-018-0579-z] and other studies, whereas the analyses on eMERGE were performed across 309 phecodes (Supplementary File `(same as in Methods/TWAS)`{.red}).
...
`(UPDATE after results/clustering)`{.red} Are these cell type-specific modules associated with _different_ diseases, thus potentially representing a "network pleiotropy" example from an omnigenic point of view [@doi:10.1016/j.cell.2017.05.038]?
