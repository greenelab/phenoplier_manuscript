...
This process generates matrices $\mathbf{\hat{L}}$ and $\mathbf{\hat{M}}$, where each trait/drug is now described by LVs.
...
**c)** LV603, termed as a neutrophil signature in the original MultiPLIER study, was associated in PhenoPLIER with neutrophil counts and other white blood cells (bottom, showing the top 10 traits for LV603 after projectiong gene-trait associations in PhenomeXcan).
...
The approach uses a latent representation (with latent variables or LVs representing gene modules) derived from a large gene expression compendium (Figure {@fig:entire_process}b, top) to integrate TWAS and with drug-induced transcriptional responses (Figure {@fig:entire_process}b, bottom) for a joint analysis.
The approach consists in three main components (Figure {@fig:entire_process}b, middle):
...
For TWAS, we used PhenomeXcan [@doi:10.1126/sciadv.aba2083] as a discovery cohort and the results from the eMERGE network as a replication cohort.
PhenomeXcan provides results for 4,091 different diseases and traits from the UK Biobank [@doi:10.1038/s41586-018-0579-z] and other cohorts (Table `S999`{.red}), whereas the analyses on eMERGE was performed across 309 phecodes (Table `S999`{.red}).
We used transcriptional responses to small molecule perturbations from LINCS L1000 [@doi:10.1016/j.cell.2017.10.049] comprising 1,170 compounds.
We obtained a latent gene expression representation from the MultiPLIER models [@doi:10.1016/j.cels.2019.04.003], which were derived by applying a matrix factorization method (the pathway-level information extractor or PLIER [@doi:10.1038/s41592-019-0456-1]) to recount2 [@doi:10.1038/nbt.3838] -- a uniformly-curated collection of transcript-level gene expression quantified by RNA-seq in a large, diverse set of samples collected across a range of disease state, cell types differentiation stages, and various stimuli (see [Methods](#sec:methods:reg)).
...
For this projection of different data modalities into the same space, PhenoPLIER converts gene associations to an LV score:
...
Although this is not strickly necessary for PhenoPLIER to work, these annotations can dramatically improve the interpretability of results.
...
To further analyze LV-trait associations, we developed a regression framework to compute an LV-trait $p$-value by fitting a model with LV weights as predictors of a trait's gene associations (Figure {@fig:entire_process}b, middle).
...
1) neutrophil counts and other white blood cell traits were ranked among the top 10 traits out of 4,091 (Figure {@fig:entire_process}c, bottom) and `UPDATE THIS`{.red} significantly associated with this LV (Supplementary Table @tbl:sup:phenomexcan_assocs:lv603) when using our regression method,
...
Thus, the gene expression space (which has no GTEx samples) and the TWAS space (with gene models trained using GTEx, see [Methods](#sec:methods:predixcan)) contained shared patterns, and the approach can link transcriptional mechanisms from large and diverse dataset collections to complex traits.
