...
MultiXcan is more powerful than PrediXcan in detecting gene-trait associations, although it does not provide direction of effects.
...
Since $\mathbf{T}^{\top}\mathbf{T}$ is singular for many genes, S-MultiXcan computes the pseudo-inverse $Cor(\mathbf{T})^{+}$ using the $k$ top principal components, and thus $\mathbf{\hat{z}}^{\top} Cor(\mathbf{T})^{+} \mathbf{\hat{z}} \sim \chi_k^2$.
<!--  -->
...
We used two large TWAS resources obtained from different cohorts for discovery and replication.
<!--  -->
PhenomeXcan [@doi:10.1126/sciadv.aba2083], our discovery cohort, provides results on 4,091 traits across different categories (Supplementary Table `XXX`{.red}).
...
Our discovery cohort was eMERGE [@doi:10.1038/gim.2013.72], where the same TWAS methods were run on 309 phecodes [@doi:10.1101/2021.10.21.21265225] across different categories (Supplementary Table `YYY`{.red}).
...
The MultiPLIER models reduced the dimensionality in recount2 to into 987 LVs.
...
For our drug repurposing and cluster analyses, we projected TWAS associations $\mathbf{M}$ (either from S-PrediXcan or S-MultiXcan) into this low-dimensional gene module space using

$$
\hat{\mathbf{M}} = (\mathbf{Z}^{\top} \mathbf{Z} + \lambda_{2} \mathbf{I})^{-1} \mathbf{Z}^{\top} \mathbf{M},
$$ {#eq:proj}

where in $\hat{\mathbf{M}}^{l \times n}$ all gene-trait associations in PhenomeXcan are now described by gene modules instead of single genes.


### A regression model for LV-trait associations

`REPLACE EPSILON WITH BOLD VERSION`{.red}
...
Since the error terms $\epsilon$ could be correlated, we cannot assume they have independent normal distributions as in a standard linear regression model.
In the PrediXcan family of methods, the predicted expression of a pair of genes could be correlated if they share eQTLs or if they are in LD.
...
As shown in Equation `XXX`{.red}, S-MultiXcan itself approximates the joint regression effect sizes and variances in MultiXcan using the marginal regression estimates from S-PrediXcan.
`this sentence is mentioned above, remove from here`{.red}
...
Additionally, S-PrediXcan estimates gene-trait associations with tissue-specific genotype covariances (using different samples of GTEx for each tissue), which complicates the derivation of an S-MultiXcan-specific solution to compute $\mathbf{R}$.
...
The model can also detect LVs associated with relevant traits (Figure `LV246`{.red}) that are replicated in different cohorts (Figure/Table `LV246 in eMERGE`{.red}).
...
The same procedure was used for the LV-based approach, where we projected $\mathbf{M}^{t}$ and $\mathbf{L}$ into our latent representation using Equation (@eq:proj), leading to $\hat{\mathbf{M}}^t$ and $\hat{\mathbf{L}}^{l \times c}$, respectively.
<!--  -->
