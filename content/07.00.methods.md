...
PhenoPLIER is a framework that combines different computational approaches to integrate gene-trait associations and drug-induced transcriptional responses with groups of funtionally-related genes (refered to as gene modules or latent variables/LVs).
...
We used Summary-PrediXcan (S-PrediXcan) [@doi:10.1038/s41467-018-03621-1] and Summary-MultiXcan (S-MultiXcan) [@doi:10.1371/journal.pgen.1007889] as the gene-based statistical approaches, which are both part of the PrediXcan family of methods [@doi:10.1038/ng.3367].
We broadly refer to these approaches as TWAS (transcription-wide association studies).
S-PrediXcan computes the univariate association between a trait and a gene's predicted expression in a single tissue, whereas S-MultiXcan computes the joint association between a gene's predicted expression in all tissues and a trait.
S-PrediXcan provides the direction of effect, which was used in our drug repurposing framework.
S-MultiXcan, on the other hand, is more powerful but only provides a $p$-value, and we used it for the regression and clustering models.
The only input needed for these TWAS approaches are GWAS summary statistics and a gene expression prediction model.


SEE WHAT IS NECESSARY TO KEEP BELOW:

- i need to mention the individual level version of MultiXcan
- i need to say something about princicpal components with S-MultiXcan
- not related: use trick by Jake to separate figures in Supplementary Material

Below we briefly provide details about these approaches that will be necessary to explain our regression framework later.
...
CONTINUE PRELIMINARIES IF NECESSARY

S-PrediXcan computes a gene-trait association for a tissue $t$

- spredixcan is tissue specific, and provides direction of effect
    - we used s-predixcan for the drug repurposing framework, since this approach provides a direction of effect
- smultixcan is not tissue specific, no directio of effect
    - we used smultixcan in our LV-trait regression model and for the clustering method
...
Using a matrix factorization approach, PLIER deconvolutes gene expression data into a set of latent variables (LV), where each represents a gene module.
This reduced the data dimensionality into 987 LVs.
...
EXPLAIN BELOW THAT WE PROJECTED SPREDIXCAN OR SMULTIXCAN FOR DIFFERENT TASKS:
...
### A regression model for LV-trait associations

We adapted the gene-set analysis framework from MAGMA [@doi:10.1371/journal.pcbi.1004219] to TWAS.
We used a competitive test to predict gene-trait associations using gene weights from an LV, testing whether top-weighted genes for an LV are more strongly associated with the phenotype than other genes with relatively small or zero weights.
Thus, we fit the model

$$
\mathbf{p}=\beta_{0} + \mathbf{s} \beta_{s} + \sum_{i} \mathbf{x}_{i} \beta_{i} + \epsilon,
$$
...
$\mathbf{x}_{i}$ is a gene property used as covariate;
$\beta$ are effect sizes (with $\beta_{0}$ as the intercept);
and $\epsilon \sim \mathrm{MVN}(0, \sigma^{2} \mathbf{R})$ is a vector of error terms with a multivariate normal distribution (MVN) where $\mathbf{R}$ is the matrix of gene correlations.

The model tests the null hypothesis $\beta_{s} = 0$ against the one-sided hypothesis $\beta_{s} > 0$.
Therefore, $\beta_{s}$ reflects the difference in trait associations between genes that are part of LV $\ell$ (within top 1% of weights) and genes that are outside of it.
...
and 2) "gene density", defined as the ratio of the number of PCs to the number of tissues available for a gene.

Since the error terms $\epsilon$ could be correlated, we cannot assume they have independent normal distributions.
In the PrediXcan family of methods, the predicted expression of a pair of genes could be corrleated if they share eQTLs or they are in LD.
Therefore, we used a generalized least squares approach instead of standard linear regression model.
The gene-gene correlation matrix $\mathbf{R}$ was approximated by computing the correlations between the model sum of squares (SSM) for each pair of genes under the null hypothesis of no association: $\mathrm{SSM}=\sum(\hat{y}_{i} - \bar{y})^2$, where $\hat{y}_i$ is the predicted value for individual $i$ and $\bar{y}$ is the mean of the phenotype.
`MAYBE CITE EQUATION HERE`{.red}
These correlations are derived from the MultiXcan model (individual-level), where the predicted expression matrix $\mathbf{T}_{i} \in \mathbb{R}^{n \times p_i}$ of a gene $i$ across $p_i$ tissues is projected into its top $k_i$ principal components, resulting in matrix $\mathbf{P}_{i} \in \mathbb{R}^{n \times k_i}$.
...
`EXPLAIN what standardized means`{.red}
Therefore, the correlation between the SSMs for genes $i$ and $j$ can be written as follows, where columns $\mathbf{P}$ are standardized:
...
The cross-correlation matrix between PCs $Cor(\mathbf{P}_{i}, \mathbf{P}_{j}) \in \mathbb{R}^{k_i \times k_j}$ is given by
...
where $\frac{\mathbf{T}_{i}^{\top} \mathbf{T}_{j}}{n-1} \in \mathbb{R}^{p_i \times p_j}$ is the cross-correlation matrix between predicted expression of genes $i$ and $j$,
...
To estimate the correlation of predicted expression between genes $i$ and $j$ in tissue models $k$ and $l$, $(\mathbf{t}_k^i, \mathbf{t}_l^j)$ ($\mathbf{t}_k^i$ is the $k$th column of $\mathbf{T}_{i}$), we used [@doi:10.1371/journal.pgen.1007889; @doi:10.1038/s41467-018-03621-1]:
...
whehre $X_a$ is the genotype of SNP $a$,
$w_a^k$ is the weight of SNP $a$ for gene expression prediction in the tissue model $k$,
and $\Gamma = \widehat{\mathrm{var}}(\mathbf{X}) = (\mathbf{X} - \mathbf{\bar{X}})^{\top} (\mathbf{X} - \mathbf{\bar{X}}) / (n-1)$ is the genotype covariance matrix using GTEx v8 [@doi:10.1126/science.aaz1776] as the reference panel, which is the same used in the PrediXcan methods.
The predicted expression variance of gene $i$ in tissue $k$ is estimated as:
...
Note that, since we used the MultiXcan regression model (Equation `INDIVIDUAL-LEVEL EQUATION`{.red}), $\mathbf{R}$ is only an approximation of gene correlations in S-MultiXcan.
As shown in Equation `XXX`{.red}, S-MultiXcan itself approximates the the joint regression effect sizes and variances in MultiXcan using the marginal regression estimates from S-PrediXcan.
For this, S-MultiXcan assumes that the variance of the error terms in the joint regression, $\sigma^2$, is approximately equal to the residual variance of the marginal regressions.
Additionally, S-PrediXcan estimates gene-trait associations with tissue-specific genotype covariances (using different samples of GTEx for each tissue), which complicates the derivation of a S-MultiXcan-specific solution to compute $\mathbf{R}$.
To account for this, we used a submatrix $\mathbf{R}_{\ell}$ corresponding to genes that are part of LV $\ell$ only (top 1% of genes) instead of the full matrix $\mathbf{R}$.
This simplification is conservative, and our null simulations (Supplementary Note `XXX`{.red}) show that it is well-calibrated and it can correct for LVs with adjacent and highly correlated genes at the top.
The model can also detect LVs associated with relevant traits (Figure `LV246`{.red}) that replicated in different cohorts (Figure/Table `LV246 in eMERGE`{.red}).

We ran our regression model for all 987 LVs in our models across the 4,091 traits in PhenomeXcan.
For replication, we ran the model in the 309 phecodes present in eMERGE.
We adjusted the $p$-values using the Benjamini-Hochberg procedure.


MAYBE SAY HERE SOMETHING LIKE "TWAS resources:
AND MENTION eMERGE and cite paper!
### PhenomeXcan: a large TWAS resource on 4,091 traits
...
THIS IS NOT NECESSARY: 2) a posterior probability of colocalization between GWAS loci and *cis*-eQTL with fastENLOC [@doi:10.1126/sciadv.aba2083; @doi:10.1101/2020.07.01.182097].
...
FOR THE REGRESSION FRAMEWORK WE USE THE PVALUES, NOT ZSCORES: For S-MultiXcan, we do not have the direction of effect, and we used the $p$-values converted to $z$-scores $\mathbf{M}=\Phi^{-1}(1 - p/2)$, where $\Phi^{-1}$ is the probit function.
