...
PhenoPLIER is a framework that combines different computational approaches to integrate gene-trait associations and drug-induced transcriptional responses with groups of functionally-related genes.
...
PhenoPLIER is a framework that combines different computational approaches to integrate gene-trait associations and drug-induced transcriptional responses with groups of functionally-related genes.
Gene-trait associations are computed using the PrediXcan family of methods, whereas latent variables are inferred by the MultiPLIER models applied on large gene expression compendia.
PhenoPLIER provides 1) a regression model to compute an LV-trait association, 2) a consensus clustering approach applied to the latent space to learn shared and distinct transcriptomic properties between
...
We used S-PrediXcan and S-MultiXcan as the two different implementations of TWAS.
S-PrediXcan is a single-sample implementation of PrediXcan while S-MultiXcan is a joint-sample implementation of MultiXcan.
...
We used S-PrediXcan and S-MultiXcan as the two different implementations of TWAS.
S-PrediXcan is a single-sample implementation of PrediXcan while S-MultiXcan is a joint-sample implementation of MultiXcan.
S-PrediXcan and S-MultiXcan only need GW
...
To arrive at this expression, S-MultiXcan uses the conservative approximation $\sigma_{e}^{2} \approx \sigma
...
The MultiPLIER [@doi:10.1016/j.cels.2019.04.003] extracts patterns of co-expressed genes from recount2 [@doi:10.1038/nbt.3838] (without including GTEx samples), a large gene expression dataset.
The approach applies the pathway-level information extractor method (PLIER) [@doi:10.1038/s41592-019-0456-1], which performs unsupervised learning using prior knowledge (canonical pathways) to reduce technical noise.
PLIER uses a matrix factorization approach that deconvolutes gene expression data into a set of latent variables (LV), where each LV represents a gene module.
The
...
-70 is less than the minimum of 0 - 'max_tokens'
-->
...
S-MultiXcan
Equation (@eq:multixcan)
$$ \mathbf{R}_{\ell} = \mathbf{R}_{\ell} - \mathbf{R}_{\mathbf{all}} $$

S-PrediXcan
Equation (@eq:spredixcan)
$$ \mathbf{R}_{\ell} = \mathbf{R}_{\ell} - \mathbf{Z}_{\mathbf{all}} $$

LV
Equation (@fig:reg:null_sim)
$$ \mathbf{R}_{\ell} = \mathbf{R}_{\ell} - \mathbf{Z}_{\mathbf{lv}} $$

Table (@tbl:sup:phenomexcan_assocs:lv246)

Table (@tbl:sup:emerge_assocs:lv246)
...
In Equation (@eq:reg:corr_genes), for each gene, we computed the correlation between the expression level of the gene and the expression level of each of the SNPs in GWAS used as input for the TWAS approaches.
This is necessary to obtain more accurate correlations estimates [@doi:10.1371/journal.pgen.1007889].

In Equation (@eq:reg:corr_genes), we computed the correlation between the expression level of the gene and the expression level of each of the SNPs in GWAS used as input for the TWAS approaches.
This is necessary to obtain more accurate correlations estimates [@doi:10.1371/journal.pgen.1007889].
...
We used spectral clustering on
...
Cell culture.
HepG2 cells were obtained from ATCC (ATCC® HB-8065™), and maintained in Eagle's Minimum Essential Medium with L-Glutamine (EMEM, Cat.
112-018-101, Quality Biology) supplemented with 10% Fetal Bovine Serum (FBS, Gibco, Cat.16000-044), and 1% Pen/Strep (G
...
The Broad GPP genome-wide Human Brunello CRISPR knockout Pooled library was provided by David Root and John Doench from Addgene (Cat.
73179-LV).
It consists of 76,441 sgRNAs, and targets 19,114 genes in the human genome with an average of 4 sgRNAs per gene.
Each 20nt sgRNA cassette was inserted into the lentiCRIS-PRv2 backbone between U6 promoter and gRNA scaffold.
Through cell transduction, the lentiviral vectors which encode Cas9 were used to deliver the sgRNA cassette containing plasmids into cells during cell replication.
...
In order to achieve a coverage (representation) of at least 500 cells per sgRNA, and at an MOI between
...
Three conditions of Genomic DNA (Un-Sorted Control, lentiV2 GFP-High, and lentiV2 GFP-Low) were extracted using QIAamp DNA Blood Mini Kit (Qiagen, Cat.51104), followed by PCR to access the quality and quantity of the gDNA.
...
The flow cell was sequenced on the HiSeq 2500.
