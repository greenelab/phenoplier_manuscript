...
In practice, given the high correlation between predicted expression values for a gene across different tissues, MultiXcan uses the principal components (PCs) of $\mathbf{T}$ to avoid collinearity issues.
<!--  -->
...
where $\hat{\mathbf{M}}^{l \times q}$ is a matrix where traits in PhenomeXcan are represented by gene modules instead of single genes.
<!--  -->
...
Our simulations (Supplementary Note `XXX`{.red}) show that the model is approximately well-calibrated and it can correct for LVs with adjacent and highly correlated genes at the top (Supplementary Figure `YYY`{.red}).
...
As suggested in [@doi:10.1038/nn.4618], $k$ could be either all genes or the top 50, 100, 250, and 500; then we averaged score ranks across all $k$ and obtain $\mathbf{D}^t$.
...
Specifically, we used *minPts* values from 2 to 125, and for each data representation (raw, PCA and UMAP), we determined a plausible range of $\epsilon$ values by observing the distribution of the mean distance of the *minPts*-nearest neighbors across all data points.
...
<!--  -->
To circumbent this, we used a supervised learning approach to detect which gene modules/LVs are the most important for each cluster of traits (Figure @fig:clustering:design b).
