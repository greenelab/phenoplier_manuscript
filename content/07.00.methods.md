...
PhenoPLIER is a framework that integrates different computational approaches to integrate gene-trait associations and drug-induced transcriptional responses with groups of functionally-related genes (referred to as gene modules or latent variables/LVs).
Gene-trait associations are computed using the PrediXcan family of methods, whereas latent variables
...
We used S-PrediXcan and S-MultiXcan to project the genetic associations through gene expression patterns.
S-PrediXcan, the summary-based version of PrediXcan, computes the univariate association between a trait and a gene's predicted expression in a single tissue.
In contrast, S-MultiXcan, the summary-based version of MultiXcan, computes the joint association between a gene's predicted expression in all tissues and a trait.
...
We used S-PrediXcan and S-MultiXcan to project the genetic associations through gene expression patterns.
S-PrediXcan, the summary-based version of PrediXcan, computes the univariate association between a trait and a gene's predicted expression in a single tissue.
In contrast, S-MultiXcan, the summary-based version of MultiXcan, computes the joint association between a gene's
...
Here we briefly provide the details about these TWAS methods that are necessary to explain our regression framework later (see the referenced articles for more information).
In the following, we refer to $\mathbf{y}$ as a vector of traits for $n$ individuals that is centered for convenience (so that no intercept is necessary); $\mathbf{\tilde{t}}_l = \sum_{a \in \mathrm{model}_l} w_{a}^{l} X_{a}$ is the gene's predicted expression for all individuals in tissue $l$, $X_a$ is the genotype of SNP $a$ and $w_{a}$ its weight in the tissue prediction model $l$; and $\math
...
Another important point is that $Cor(\mathbf{T})$ is estimated using a global genotype covariance matrix, whereas marginal $\hat{z}_l$ in Equation (@eq:spredixcan) are approximated using
...
PhenomeXcan [@doi:10.1126/sciadv.aba2083], our discovery cohort, provides results on 4,091 traits across different categories.
Supplemenetary File 1 has all the details about the included GWAS, sample size and disease/trait categories.
In PhenomeXcan, these publicly available GWAS summary statistics were used to compute 1) gene-based associations with the PrediXcan family of methods (described before), and 2) a posterior probability of colocalization between
...
We applied the models to a set of 1,000 disease-associated gene expression datasets from the human disease literature, and found that the models were able to identify disease etiology and drug mechanisms with high accuracy.
...
We then used the Fisher's exact test to compare the difference in the log-likelihoods of the two models.
The results showed that, for both gene size and gene density, genes within LV $\ell$ are more likely to be associated with traits than genes outside of it.
This suggests that the genes within LV $\ell$ are more likely to be responsible for the observed associations, and that the differences in association between genes within LV $\ell$ and genes outside of LV $\ell$ are due to their different expression patterns.
...
This resulted in a total of 8 correlation matrices.
For eMERGE, we only used GWAS that were downloaded from the eMERGE website and were not included in the S-PrediXcan results.
This resulted in a total of 3 correlation matrices.
...
In PhenomeXcan, most of the GWAS (4,049) were obtained from the UK Biobank using the same pipeline and including the same set of SN
...
We first performed a Stouffer's method to combine results in $\mathbf{M}$ (with $p$-values converted to $z$-scores, as described before) for traits that mapped to the same EFO term.
...
We then used a k-means clustering algorithm on the $\mathbf{D}$ similarity matrix to group the data into clusters.
The clusters were then used to determine the functional annotation of the genes within them.
We used the Kyoto Encyclopedia of Genes and Genomes (KEGG) pathway database to find the most significant pathways within each cluster.
We then used the Drug Repurposing Database (DRD) to find the most significant drug targets within each cluster.
Finally, we used the Gene Ontology (GO) to find the most significant molecular functions within each cluster.

We then used a k-means clustering algorithm on the $\mathbf{D}$ similarity matrix to group the data into clusters.
The clusters were then used to determine the functional annotation of the genes within them.
We used the Kyoto Encyclopedia of Genes and Genomes (KEG
...
A genome-wide pooled CRISPR-Cas9 library was generated by David Root and John Doench from Addgene (Cat.
...
The Broad GPP genome-wide Human Brunello CRISPR knockout Pooled library was used as a reference for gene expression analysis.
This library was generated by genotyping CRISPR-Cas9 knockouts in over 1,000 individuals from the Broad GPP cohort.
This library was used to identify gene expression changes in response to various treatments.

A genome-wide pooled CRISPR-Cas9 library was generated by David Root and John Doench from Addgene (Cat.
73179-LV), and was used for HepG2 cell transduction.
It consists of 76,441 sgRNAs, and targets 19,114 genes in the human genome with an average of 4 sgRNAs per gene.
Each 20nt sgRNA cassette was inserted into the lentiCRIS-PRv2 backbone between U6 promoter and gRNA scaffold.
Through cell transduction, the lentiviral vectors which encode Cas9 were used to deliver the sgRNA cassette containing
...
**After transduction, cells were collected, pooled, and replated.**
...
The cell pellet was spun down at 500 x g for 5min at 4oC.
The dry pellet was kept at -80oC for further genomic DNA isolation.
The rest of the cells (approximately 200M) were kept in 100mm dishes and stained with a fluorescent dye (LipidSpotTM 488,
...
Figure @fig:sup:crispr:fig3: PCR of sgRNA cassette and lentiviral specific transgene in isolated gDNA.

The verification of the sgRNA cassette and lentiviral specific transgene in isolated gDNA was done through PCR.
The results of the PCR showed that the sgRNA cassette and lentiviral specific transgene were correctly inserted into the gDNA.

The flow cell was sequenced on the HiSeq 2500 at the Broad Institute.
