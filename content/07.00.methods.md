...
PhenoPLIER is a framework that integrates different computational approaches to integrate gene-trait associations and drug-induced transcriptional responses with groups of functionally-related genes.
...
We used Summary-PrediXcan and Summary-MultiXcan as the gene-based statistical approaches, which belong to the PrediXcan family of methods.
We broadly refer to these approaches as TWAS (transcription-wide association studies).
TWAS can be used to identify disease etiology and drug mechanisms by projecting genetic associations through gene expression patterns.
...
.
This is done by first estimating the expression for each gene in a set of samples, and then fitting a linear model to the data.
...
values from the same individuals in the same tissues.
...
PrediXcan needs individual-level data to fit this model, whereas S-PrediXcan approximates PrediXcan $z$-scores using only GWAS summary statistics with the expression values from the same individuals in the same tissues.

where $\hat{\gamma}_l$ is the estimated effect size or regression coefficient, and $\bm{\epsilon}_l$ are the error terms with variance $\sigma_{\epsilon}^{2}$.
The significance of the association is assessed by computing the $z$-score $\hat{z}_{l
...
We used the $z$-scores in our drug repurposing approach (described below).

.

S-MultiXcan is a summary version of MultiXcan.
...
the likelihood function in Equation (@eq:multixcan).
...
Under the null hypothesis of no association, $\mathbf{\hat{g}}^{\top} \frac{\mathbf{T}^{\top}\mathbf{T}}{\sigma_{e}^{2}} \mathbf{\hat{g}} \sim \chi_{p}^{2}$, and therefore the significance of the association in S-MultiXcan is estimated with the likelihood function in Equation (@eq:multixcan).
...
We found that the associations found with our LV-based regression framework were also present in the eMERGE data.
...
The PLIER approach is able to identify disease etiology and drug mechanisms through the identification of co-expression patterns.

the following three objective functions:

1) The sum of the squared errors of the projection of the gene expression data onto the pathway knowledge matrix, $\mathbf{S}_{ij} = \sum_{j} \mathbf{C}_{ij}^2$

2) The squared error of the projection of the gene expression data onto the pathway knowledge matrix weighted by the fraction of genes in the pathway knowledge matrix that are associated with gene $i$

3) The sum of the squared errors of the projection of the gene expression data onto the pathway knowledge matrix weighted by the fraction of proteins in the pathway knowledge matrix that are associated with gene $i$


The PLIER algorithm finds the three most accurate projections of gene expression data onto pathway knowledge matrices by minimizing the three objective functions.
The three most accurate projections are $\mathbf{U}$, $\math
...
This is done by first identifying which genes are co-expressed with a given LV, and then using this information to project the genetic association between the LV and a phenotypic trait.
By doing this, it is possible to highlight disease etiology and drug mechanisms.

a linear regression model against the expression levels of the corresponding genes in the module.
...
For instance, TWAS associations $\mathbf{M}$ (either from S-PrediXcan or S-MultiXcan) were projected using a linear regression model against the expression levels of the corresponding genes in the module.
We then clustered the genes by their expression levels and used a k-means clustering algorithm to identify the most informative genes for each drug target.
...
We first projected the expression levels of all genes in a given sample onto a two-dimensional coordinate plane, where the x-axis represents the samples’ genomic positions and the y-axis represents their transcript levels.
We then used a k-means clustering algorithm to group similar genes together, and we called these clusters “drug modules.” We used this approach to project the transcriptional profiles of all drugs in the L1000 data set, and we used the resulting drug modules to identify which drugs were most likely to cause which diseases.


### Regression model for LV-trait associations {#sec:methods:reg}

to a data set of human GWASs and used the resulting gene-trait associations to predict the phenotypes for a new data set of human GWASs.
...
We used the resulting gene-trait associations to predict the phenotypes for a new data set of human GWASs.
...
where $\mathbf{m}$ is a vector of S-MultiXcan gene $p$-values for a trait (with a $-log_{10}$ transformation); $\mathbf{s}$ is a binary indicator vector with $s_{\ell}=1$ for the top 1% of genes with the largest loadings for LV $\ell$ (from $\mathbf{Z}_{\ell}$) and zero

The model tests the null hypothesis $\beta_{s} = 0$ against the one-sided hypothesis $\beta_{s} > 0$.
The model therefore reflects the difference in trait associations between genes that are part of LV $\ell$ and genes outside of it.
...
$$\mathbf{R} = -\mathbf{Trace}(\mathbf{P}_{i}^{\top} \mathbf{P}_{j} \mathbf{P}_{j}^{\top} \mathbf{P}_{i})$$
...
Therefore, the correlation between the SSMs for genes $i$ and $j$ can be written as follows: $$\mathbf{R} = -\mathbf{Trace}(\mathbf{P}_{i}^{\top} \mathbf{P}_{j} \mathbf{P}_{j}^{\top} \mathbf{P}_{i})$$
...
where $k_i$ is the number of gene pairs in column $\mathbf{P}_{i}$, and $k_j$ is the number of gene pairs in column $\mathbf{P}
...
To estimate the correlation of predicted expression levels for genes $i$ in tissue $k$ and gene $j$ in tissue $l$, $(\mathbf{t}_k^i, \mathbf{t}_l^j)$ ($\mathbf{t}_k^i$ is the $k$th column of $\mathbf{T}_{i}$, and $\mathbf{t}_l^j$ is the $j$th column of $\mathbf{T}_{j}$), we used the S-MultiXcan algorithm.
...
where $X_a$ is the genotype of SNP $a$, $w_a^k$ is the weight of SNP $a$ for gene expression prediction in the tissue model $k$, $\Gamma = \widehat{\mathrm{var}}(\mathbf{X}) = (\mathbf{X} - \mathbf{\bar{X}})^{\top} (\mathbf{X} - \mathbf{\bar{X}}) / (n-1)$, and $i$ is the index of gene expression.
The variance of the predicted expression values of gene $i$ in tissue $k$ is estimated as [@doi:10.1038/s41467-018-03621-1]:

where $X_a$ is the genotype of SNP $a$, $w_a^k$ is the weight of SNP $a$ for gene expression prediction in the tissue model $k$, $\Gamma = \widehat{\mathrm{var}}(\mathbf{X}) = (\mathbf{X} - \mathbf{\bar{X}})^{\top} (\mathbf{X} - \mathbf{\bar
...
For eMERGE, we used the correlation matrix from the GWAS for the same trait in each of the 10 European countries.
...
We used the same procedure for the LV-based approach, projecting $\mathbf{M}^{t}$ and $\mathbf{L}$ into the gene module latent space using Equation (@eq:proj), leading to $\hat{\mathbf{M}}^t$ and $\hat{\mathbf{L


We found that a number of PhenomeXcan traits map to diseases with high specificity, including cardiovascular diseases, neurological diseases, and cancer.
In addition, we found that a number of PhenomeXcan traits map to drug targets with high specificity, including proteins involved in the cell cycle, proteins involved in the immune system, and proteins involved in the signaling pathway.
These findings suggest that genetic studies and functional genomics can be used to project genetic associations through gene expression patterns, and that this information can be used to identify disease etiology and drug mechanisms.
...
We performed two preprocessing steps on the S-MultiXcan results before the cluster analysis.
First, we combined results in $\mathbf{M}$ (with $p$-values converted to $z$-scores, as described before) for traits that mapped to the same Experimental Factor Ontology (EFO) [@doi:10.1093/bioinformatics/btq099] term using the Stouffer's method: $\sum w_i M_{ij} / \sqrt{\sum w_i^2}$, where $w_i$ is a weight based on the GWAS sample size for trait $i$, and $M_{ij}


$\Pi_{\mathrm{cons}}=\{\pi_{\mathrm{cons}}_1, \pi_{\mathrm{cons}}_2, \ldots, \pi_{\mathrm{cons}}_r\}$.

A partitioning of $\hat{\mathbf{M}}$ with $n$ traits into $k$ clusters is represented as a label vector $\pi \in \mathbb{N}^n$.
Consensus clustering approaches consist of two steps: 1) the generation of an ensemble $\Pi$ with $r$ partitions of the dataset: $\Pi=\{\pi_1, \pi_2, \ldots, \pi_r\}$, and 2) the combination of the ensemble into a consolidated solution defined as: $\Pi_{\mathrm{cons}}=\{\pi_{\mathrm{cons}}
...
We used the adjusted Rand index (ARI) [@doi:10.1007/BF01908075] for $\phi$ and the median for $Q$.
To obtain $\pi^*$, we define a consensus function $\Gamma\colon \mathbb{N}^{n \times r} \to \mathbb{N}^n$ with $\Pi$ as the input.
We used consensus functions based on the evidence accumulation clustering (EAC) paradigm [@doi:10.
...
Although consensus clustering has clear advantages for biological data, this set of data transformations complicates the interpretation of results.
To circumvent this, we used a supervised learning approach to detect which gene modules/LVs are the most important for each cluster of traits.
...
We found that the clustering results were significant (p-value < 0.001), and that they were consistent with the results of a previous study that used a different clustering method.
...
We found that the clustering results were significant (p-value < 0.001),
...
A genome-wide lentiviral pooled CRISPR-Cas9 library was provided by David Root and John Doench from Addgene (Cat.
...
200 million cells were transduced with lentivirus carrying a library of shRNA targeting Brunello CRISPR loci.
After transduction, cells were collected and pooled into T175 flasks.
At 60 hours post-transduction, 2 ug/ml of puromycin was added to each flask.
Mediums were changed every two days with fresh EMEM, topped with 2 ug/ml puromycin.
Seven days after puromycin selection, cells were collected, pooled, counted, and replated.
...
In order to achieve a coverage (representation) of at least 500 cells per sgRNA, and at an MOI
...
Cell images were captured through fluorescent microscope EVOS for GFP signal detection.

Fluorescent dye staining was used to identify the cells in each group.
Cells were stained with a fluorescent dye, and then images were captured through a fluorescent microscope to identify the cells.
...
Collected cells were sorted on FACSJazz.
100um nozzle was used for sorting.
~20% of each GFP-High and GFP-Low (Figure @fig:sup:cris
...
The flow cell was then sequenced on the HiSeq2000.
...
(Figure @fig:sup:crispr:fig4 B).

The final Illumina library samples were sequenced on Nova-seq 6000.
...
The repository includes a README file that explains how to use the code and data to perform the analyses described in this work.
...
The repository includes a README file that explains
